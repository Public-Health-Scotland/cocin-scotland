...
hospitaltests <- data %>%
  ...
  # select only required variables for this section
  select(
    chi_number, adm_date, abo, bmi, ct_res, ct_res_sp, ct_us_ecg, cxr, cxroth_sp,
...
medications <- data %>%
  ...
  # select only required variables for this section
  select(
    chi_number, adm_date, nebu, prone, trialdrugs, study_convpl, study_oth,
...
IMOVE_data <- list(
  demographics, hospinfo, patientinfo, symptoms, hospitaltests,
