...
genomics_hospdata <- data %>%
  ...
  mutate(ethnic_group = ifelse(substr(ethnicity_group, 1, 1) == "1",
    ...
    ifelse(substr(ethnicity_group, 1, 1) == "2",
      ...
      ifelse(substr(ethnicity_group, 1, 1) == "3",
        "Asian, Asian Scottish or Asian British",
        ifelse(substr(ethnicity_group, 1, 1) == "4",
          "African",
          ifelse(substr(ethnicity_group, 1, 1) == "5",
            "Caribbean or Black",
            ifelse(substr(ethnicity_group, 1, 1) == "6",
              "Other ethnic group",
              ifelse(ethnicity_group == "98",
                "Refused/Not provided by patient",
                ifelse(ethnicity_group == "99",
                  "Not Known", NA
                )
              )
            )
  ...
  rename(
    ...
    Ethnicity_cocin = ethnicity_cocin,
    Ethnicity_Group_cocin = ethnicity_grouped_cocin,
    AdmitHosp = adm_date,
    Admit_ICU = icu,
  ...
  select(
    CHI, ECOSSID, First_name, Surname, Sex, Postcode, HB_Residence, HB_Treatment,
    DOB, Ethnicity, Ethnicity_Group, Ethnicity_cocin, Ethnicity_Group_cocin,
    Admit_Hosp,
