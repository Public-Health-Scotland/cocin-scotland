...
## Librarys
library(tidyverse)
scot_data <- readr::read_rds("scottish_data_prepped.rds.gz")
...
studyidentifiers <- scot_data %>% 
  group_by(subjid) %>% 
  summarise(hospitalcode = first(na.omit(hospid)),
            idcountry = "Scotland",
...
patientchar <- scot_data %>% 
  group_by(subjid) %>% 
  mutate(age_y = case_when(age_estimateyearsu == "Years" ~ first(na.omit(age_estimateyears))), 
...
caseseverity <- scot_data %>% 
  group_by(subjid) %>% 
  mutate(lab_covtest  = case_when(any(mbperf == "YES") ~ "Yes"), 
         labcovid = case_when(any(corna_mbcat == "YES - Confirmed") ~ "Confirmed", 
         ...
         ventilation = case_when(any(noninvasive_proccur == "YES") ~ "NonInvasive", 
  ...
  summarise(
    ...
    bilat_pneu = NA,
    pneu_any = case_when(any(vrialpneu_ceoccur =="YES")~ "Vrialpneu",
...
rkclosecontact <- scot_data %>%
  group_by(subjid) %>% 
  summarise(closecont = first(na.omit(symptoms_epi_physical)), 
            closecont_sp = NA, 
            closecont_type = NA)
...
rkexamlab <- scot_data %>%
  group_by(subjid) %>%
  summarise(ct_us_ecg = case_when(any(xray_prperf == "YES") ~ "Yes", any(xray_prperf == "NO") ~ "No" ),
            ct_res = NA,
            ct_res_sp = NA,
...
completedataset = list(studyidentifiers, hospitalward, patientchar, caseseverity, 
     rkclosecontact, rkexamlab, underlyingcc, rkhospmedc, vaccine) %>% 
      reduce(merge, by = "subjid") 
    
completedataset <-  select(completedataset,-c(1))

rm(studyidentifiers, hospitalward, patientchar, caseseverity, 
   rkclosecontact, rkexamlab, underlyingcc, rkhospmedc, vaccine)
